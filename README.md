...
| 부분 | 상태 | 비고 |
|---|---|---|
| 호출어(KWS) | 🔴 **지금 안 깨어남** | 호출어를 `하이 티드` 로 바꿨는데 분류기는 아직 재하봇용 v4 다. **v6 학습 대기** — 데이터·노트북은 완성(`tools/colab_wake_train_v6.ipynb`). v5 는 v4 에 져서 기각 |
| STT | ✅ 동작 | **`medium`**, 젯슨 GPU<sup>*</sup> |
...
<sup>*</sup> 예전에 여기 적혀 있던 `large-v3-turbo` · 1.59s · CER 2.96% 는 **다른 모델 크기의**
수치다. 운영값은 `medium`(`configs/model_paths.yaml`)이고 그 크기의 CER 은 아직 안 쟀다.
그리고 모든 STT 수치는 **9세 음성**(AI-Hub #003) 기준이다 — 실제 대상인 2~3세 녹음으로는
아직 검증하지 못했다. 이 프로젝트에서 가장 큰 미검증 영역이다.
...
2. **호출어 v6 학습** — `tools/colab_wake_train_v6.ipynb` 을 Colab 에서. 데이터는 완성돼 있다
   (`output/jaehabot_v6.tar.gz`, 341MB). 넣기 전까지 봇은 호출로 깨어나지 않는다
3. **실음성 녹음** — 학습 데이터에 실제 사람 목소리가 **한 건도 없다.** 재하 30~50 /
   부모 20~30 / 거실 소음 1시간. TV 앞에서 못 깨우는 문제는 이것 없이는 안 풀린다
4. **AI-Hub 3~6세 데이터 확보** — 지금까지의 모든 STT 수치가 9세 프록시다
5. 놀이 카드 확대 + 껍데기 놀이 3개 구현
6. 비전(STEP 9)
