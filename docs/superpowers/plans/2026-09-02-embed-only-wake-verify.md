...
> **진행 상황 (2026-09-11):** Task 0 완료 + whisper 와 A/B. 임베딩 검증이 창이 짧아 동작하지 않던 것을
> **고쳤다**(`45cc2c9`, embed_window_s·embed_settle_s). 젯슨 재확인에서 A/B 와 같은 결과 — 호출 25/35·헛깨움 1회
> (컷 0.85) / 21/35·0회(0.88) vs whisper 23/35·2회. 남은 것: 컷 선택과 Task 4(젯슨 실기).
>
...
- [x] **Step 2: 실제로 쓸 사람 목소리로 본보기를 등록한다**
...
1단계를 못 넘은 녹음은 🔴 로 따로 표시되고 통계에서 빠진다 — 그건 임베딩 컷으로는
못 고치는 실패라서다.

> **예행연습(2026-09-09, 노트북).** 도구가 맞게 재는지 있는 어른 20건으로 확인했다.
> 또박또박 5개로 등록하고 나머지 15개를 채점 → **최저 0.873**, 중앙 0.914.
...
- [x] **Step 5: 컷을 정하고 근거를 적는다**

컷 = (소음 최대) 와 (진짜 최저) 사이. 둘이 겹치면 **임베딩 단독은 성립하지 않는다** — 그 사실을 `reports/` 에 적고 이 계획을 중단한다. 겹치지 않으면 그 수를 Task 4 에서 설정에 넣는다.

> **결과 (2026-09-11, 젯슨 ReSpeaker).** 거실 45.0분 + 호출 20건(`data/wake_real/adult_20260911_1144`,
> 또박또박 5건 등록 / 15건 채점).
>
> | | 유사도 |
> |---|---|
> | 소음 최대 (하한) | **0.872** (p99 0.845, 중앙 0.787 — 1단계 후보 588프레임, 784/시간) |
> | 진짜 호출 최저 (상한) | **0.889** (1단계 통과 11건, 중앙 0.912) |
> | **여유** | **0.017** |
>
> 컷별로 보면:
>
> | 컷 | 헛깨움(45분 기준, 프레임 수라 상한) | 진짜 호출 |
> |---|---|---|
> | 0.85 | 2.7회/시간 | 11/11 |
> | 0.87 | 1.3회/시간 | 11/11 |
> | **0.88** | **0** | **11/11** |
> | 0.90 | 0 | 8/11 |
>
> **겹치지 않는다 — 형식상 Task 4 로 갈 수 있다. 다만 여유가 0.017 이다.**
> 3분 표본의 소음 최대 0.814 가 45분에서 0.872 로 올랐다. 녹음이 길수록 최대는 더 오른다
> (극값은 표본 크기를 따라 커진다). 며칠 켜 두면 0.889 를 넘는 소음이 나올 가능성이 높다.
> 화자 쪽도 흔들린다 — 08-26 녹음을 그 날의 본보기로 잰 최저가 0.873 이었으니, 0.88 컷이면
> 그 날의 호출 하나는 막힌다. **컷 0.88 은 이 45분에만 맞춘 값일 수 있다.**
>
> 부수 발견: 흘려서 5건 중 4건은 1단계에서 이미 탈락했다(점수 0.007~0.037). 임베딩 단독으로
> 바꿔도 이 4건은 안 살아난다 — 1단계 감지기의 문제다. 그 4건 중 3건을 whisper 는
> "안녕히계세요" 로 적었다(08-27 환각과 같은 증상).

> **🔴 A/B (2026-09-11, 봇과 같은 감지기로 — `reports/wake/ab_verify_20260911.md`).**
> 위 0.889 는 **파일 전체**로 잰 값이라 봇의 실제 검증 경로와 다르다. 봇 경로로 재자 둘이 드러났다.
>
> 1. **지금 코드의 임베딩 검증은 절대 통과하지 못한다.** 검증창 2.0초(25프레임)에서 임베딩이
>    10개만 나오는데 본보기 대조는 16개를 요구해 유사도가 늘 0 이다(판정 72번 전부 0.000).
>    Task 1 의 embed 모드와 08-27 보강 둘 다 해당 — **이대로 켜면 봇이 영영 안 깨어난다.**
> 2. 창을 늘려도 검증 시점이 이르다(후보 뒤 0.24초). 1단계 최고점은 0.3~0.5초 뒤에 온다.
>
> 고친 조건(임베딩 창 3초 + 임베딩 쪽만 0.48초 더 듣기)으로 whisper 와 비교:
>
> | | whisper (지금 봇) | 임베딩 0.85 | 임베딩 0.88 |
> |---|---|---|---|
> | 호출 (35) | 23 | **25** | 21 |
> | 헛깨움 45분 | 2 (2.67/h) | 1 (1.33/h) | **0** |
>
> **Task 4 전에 코드 수정이 먼저다**: 임베딩 검증창을 whisper 창과 분리해 2.5초 이상으로,
> 임베딩 쪽 settle 을 따로 두고, 실제 길이의 창으로 시험을 넣는다.
