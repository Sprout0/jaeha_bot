...
#### 2026-09-16 노트북 — 사람 없이 실제 서버로 대화 구간 전체(합성 발화를 마이크 자리에 실시간 속도로)

도구: scratchpad `sim.py`(marin 으로 만든 아이 말 → 16k → `Conversation` → 실제 Realtime). 사람 목소리·실제 스피커는 아니다.

| 턴 | 받아 적은 말 | 판단 | 체감(실제 말끝→첫 소리) | 꼬리 | 받아적기 | 요청→첫소리 |
|---|---|---|---|---|---|---|
| 1 | 안녕 틴들, 나 오늘 공원에서 강아지 봤어 | chat | 2.91s | 1.67 | 0.69 | 0.53 |
| 2 | 우리 동물 소리 놀이 하자 | game_start | 3.12s | 1.65 | 0.58 | 0.89 |
| 3~6 | 멍멍 / 야옹 / 음매 / 이제 그만할래 | game | 2.80~2.84s | 1.60~1.75 | 0.55~0.63 | 0.53~0.66 |
| 7 | 탈자(="잘 자") | chat | 3.03s (+맞장구) | 1.69 | 0.55 | 0.80 |

- **체감 2.8~3.1s** — 로컬 실기 4.14s(09-07) 대비 약 −1.1~1.3s. 턴당 $0.0029~0.0052.
- **R1(받아적기 대기) 0.55~0.69s** — 모든 턴에 붙지만 감당할 만하다. 먼저 답 시작하는 대안은 아직 필요 없다.
- R3(그대로 말하기) 문제없음 — "상어가족 틀어 줄게!" 와 끊김 문구를 글자 그대로 읽었다.
- 고정 문구 8개 첫 생성 31.3s(한 번만).

##### 발생 1 — 체감 계측 결함(고침, bab6e12)
- 증상: 첫 실행 체감이 1.11~1.39s 로 나왔다.
- 원인: `speech_stopped` 는 무음 대기 1.2초가 **끝난 뒤** 오고 `audio_end_ms` 도 대기를 포함한다(실측 실제 말끝 4,209ms → 5,504ms).
- 처리: 보낸 오디오 위치→벽시계 기록으로 `audio_end_ms − silence_ms` 를 실제 말끝으로 되돌린다. `vad_tail_s` 를 따로 남긴다.

##### 발생 2 — R2 현실화: 놀이 턴에서 모델이 대본을 벗어난다 (보류)
- 증상: 놀이 6턴 중 2턴에서 필수 낱말이 빠졌다. 시작에서 상태기계는 "강아지·멍" 인데 봇은 "사자는 어흥?", 다음 문제 "양·매애" 자리에 "오리도 꽥?". **아이가 들은 문제와 상태기계의 정답이 어긋난다.**
- 원인: 로컬은 렌더 결과에 필수 낱말이 없으면 템플릿으로 바꿔 말했다. Realtime 은 소리가 먼저 나가 바꿀 수 없다.
- 대안(택1): ① 놀이 턴은 템플릿 문장을 `say_exactly` 로 — 정확, 말투 다양성 잃음, 지연 같음 ② 놀이 턴만 글자가 다 올 때까지 소리 보류 → 빠졌으면 취소하고 템플릿 — 정확+다양, 놀이 턴 지연 증가 ③ 지시문을 더 강하게 — 확률만 낮춘다.

##### 발생 3 — 합성 발화 받아적기 오인 (관찰)
- "동물 소리 놀이 하자" → "움월 소리놀이 하자"(1차), "잘 자" → "탈자". 놀이 시작·잠들기를 놓치고 자유대화로 갔다. 1차에서는 모델이 **놀이를 하는 척** 했다.
- 합성음이라 사람 목소리와 다를 수 있다 — 젯슨 실기에서 사람 목소리로 다시 본다.
