...
#   '하이 티드': 1.15 에서 **67~70%**                  -> 상한 1.15 로 두되 수율을 감안한다
#
# 🔴 정정(2026-08-25). 처음엔 1.15 를 92% 로 적었는데 **틀렸다.** 그건 n=12 짜리
#    표본이었고, 같은 조건 60개짜리 측정 둘이 서로 일치하며 이를 뒤집었다:
#      wake_qc 표본 240개(층화)      x1.15 통과 70% (42/60)
#      스텝 A/B 60개 x 2             x1.15 통과 67% (40/60), 12스텝·24스텝 **동일**
#    확산 스텝은 원인이 아니었다(24스텝은 자모거리 중앙만 0.225 -> 0.062 로 좋아지고
#    통과/탈락 경계는 안 움직인다). 원인은 **시드 편차**다 — 빠른 합성에서는 초기
#    노이즈에 따라 음절을 삼키기도 하고 안 삼키기도 한다.
#    교훈: n=12 로 통과율을 주장하지 말 것. 이 프로젝트에서 두 번째 같은 실수다.
#
# 🟢 그래도 1.15 를 남기는 이유: 이 칸은 **진짜로 빨리 발음한 소리**다(조음 축약 포함).
#    WSOLA 로 늘린 가짜 빠르기와 다르다 — v4 실기에서 '빠르게' 통과가 10% 였던 게
#    바로 그 차이였다("합성 배속과 진짜 빠른 말은 다르다").
#    수율 70% 는 QC 가 걸러 주면 그만이다(재하봇의 42~50% 보다 낫다). 다만 **칸이
#    덜 차므로** QC 뒤 sanity_wake_data 로 칸별 개수를 보고 1.15 만 보충 생성할 것.
#    길이 실측: x0.85 1.04s -> x1.15 0.80s. 합성만으로 1.3배 폭이 나온다.
#
# ⚠️ 문구별로도 갈린다(x1.15, 60개): '하이 티드!' 83~92% / '하이티드!' 83%
#    / '하이 티드'·'하이티드' 58% / '하이 티드야' 42~50%(가장 길어서 가장 많이 삼킨다).
...
#
# 🟢🟢 v6(2026-08-25) — **낮추는 칸의 역할이 바뀌었다. 이제 '더하는' 게 아니라 '지키는' 것이다.**
#
#   v6 는 저역을 증강이 아니라 **화자 선택**으로 채웠다(M5 97Hz · M2+M5 99Hz · M3 122Hz 등,
#   12화자 중 4종이 저역). 그래서 낮추는 칸이 필요 없다고 판단했다가, 실제 클립 400개의
#   F0 를 재서 격자별로 모의해 보니 **틀렸다.** 7칸에는 낮추는 칸이 없어서
#   (5칸은 올리고 2칸은 유지) 증강이 분포를 통째로 위로 민다:
#
#     구성            중앙    5%    <=116Hz(아빠)  <=130Hz   >=220Hz(재하)
#     증강 없음       192Hz   94Hz     19.0%       24.5%     36.8%
#     7칸 (v4)       214Hz   99Hz     12.4%       18.9%     47.5%   <- 저역이 희석된다
#     10칸 (채택)     198Hz   93Hz     15.8%       21.8%     40.8%
#     11칸 (v5)      195Hz   86Hz     17.4%       23.0%     39.8%
#
#   7칸으로 가면 화자로 어렵게 만든 저역이 증강에서 도로 사라진다 — v4 가 실패한 방식 그대로다.
#
#   ⚠️ v5 가 v4 에 진 것과 모순되지 않는다. **전제가 다르다.** v5 는 F0 가 위로 쏠린
#      v4 말뭉치(중앙 165Hz) 위에 저역 칸을 얹어 **없던 봉우리**를 만들었다. v6 는 말뭉치가
#      이미 균형이라 같은 칸이 봉우리를 만드는 게 아니라 **유지**한다.
#
#   ❌ (1.0, 0.75) 만 뺐다. 우리 최저 화자가 97Hz(M5)라 x0.75 면 **73Hz** — 이 집 누구도
#      내지 않는 소리다. 실제로 11칸은 5%tile 이 86Hz 까지 내려간다(10칸은 93Hz = 원본과 같다).
#
#   ⚠️ 이 판단은 **F0 하나만** 본 것이다. v5 가 진 또 다른 이유였던 제로섬(칸이 늘면 칸당
#      표본이 얇아진다)은 이 모의로 안 보인다. 10칸도 7칸보다 얇다(596 vs 851).
#   ⚠️ 재하 F0 는 여전히 **미측정**이다. >=220Hz 를 아이 음역으로 본 것은 추정일 뿐이다.
EXPAND_GRID = [
    ...
    (1.0, 0.80),                 # 낮기만 — 성인 남성 음역(실측 104~116Hz)
    (1.15, 0.80), (1.35, 0.80),  # 낮고 빠름 — 어른이 낮은 목소리로 빨리 부르는 경우
]
#
# 🔴🔴 **여기 숫자는 명령값이지 결과가 아니다** (2026-08-25 실측, 음성 120개).
#    pitch_shift 는 **내리는 쪽에서만** 명령보다 덜 내려간다. 올리는 쪽은 정확하다.
#
#      명령   실제배율   최저화자 97Hz 가        |   명령   실제배율
#      0.75    0.779        76Hz               |   1.15    1.173
#      0.80    0.842        82Hz  <- 채택       |   1.35    1.307
#      0.85    0.908        88Hz               |
#      0.90    0.958        93Hz               |
#
#    ⚠️ 버그가 아니다. **순수 사인파로는 정확하다**(0.85 -> 0.858, 0.75 -> 0.755).
#       측정기의 탐색 하한 탓도 아니다(70Hz -> 40Hz 로 내려도 0.908 -> 0.904).
#       음성에서만 나타난다 — WSOLA 가 파형 유사도로 겹칠 자리를 찾을 때 짧은 주기 쪽으로
#       정렬되는 것으로 보인다(사인파는 정상 신호라 이 현상이 없다).
#
#    이 편향을 모르고 0.85 를 쓰면 실제로는 0.91 이 걸려 저역이 안 지켜진다.
#    실측: 0.85 로 증강한 말뭉치의 <=116Hz 비중이 예측 15.8% 가 아니라 **11.4%** 였다
#    (증강 전 19.0%). 그래서 의도한 0.85 를 얻으려고 **0.80 을 명령한다**.
#    ➡️ 이 격자를 손볼 때는 tools/ 의 F0 측정으로 **결과를 다시 재고** 정할 것.
# 클립 하나당 이만큼만 무작위로 골라 건다. 말뭉치 전체로는 위 칸들이 고루 덮이면서
# 총 개수는 v3·v4(원본×4)와 같게 유지된다 — 학습 시간이 그대로여야 비교가 된다.
# ⚠️ 칸이 7 -> 10 으로 늘었으므로 칸당 표본은 그만큼 얇아진다(v6 기준 851 -> 596).
